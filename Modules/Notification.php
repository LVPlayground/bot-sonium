...
class Notification extends ModuleBase
{
	/**
        ...
        const   NOTIFICATION_MESSAGE_LIMIT      = 8;
        ...
        public function onChannelPrivmsg (Bot $bot, $channel, $nickname, $message)
        {
                ...
                if (substr ($message, 0, strlen (self :: NOTIFICATION_COMMAND_NAME)) == self :: NOTIFICATION_COMMAND_NAME)
                {
                        ...
                        {
                                $bot -> send ('PRIVMSG ' . $channel . ' :10Warning: The message could not be stored, because there are already ' . self :: NOTIFICATION_MESSAGE_LIMIT . ' messages waiting for ' . $nickname);
                                return true;
                        }
                        ...
                        $bot  -> send ('PRIVMSG ' . $channel . ' :' . $nickname . ', okay.');
                        $this -> save ();
                ...
                if (isset ($this -> notifications [$nickname]))
                {
                        foreach ($this -> notifications [$nickname] as $index => $notification)
                        {
                                ...
                                unset ($this -> notifications [$nickname][$index]);
                        }
                        
                        if (count ($this -> notifications [$nickname]) == 0)
                        {
                                unset ($this -> notifications [$nickname]);
                        }
