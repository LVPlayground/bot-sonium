...
class Tracker extends ModuleBase
{
        ...
        public function __construct ()
        {
                ...
                echo '[Tracker] Loaded the Tracker module (' . count ($this -> m_aFeedList) . ' feeds).' . PHP_EOL;
        ...
        {
                $pContext = stream_context_create (array
                (
                        'http'  => array
                        (
                ...
                if ($sFeedData !== false)
                {
                        ...
                        if ($pReader === false) /** syntaxical error **/
                                return $aFeedItems;
                        ...
                        if (count ($pItems) > 0)
                        {
                                foreach ($pItems as $pItemInfo)
                                {
                                        $aFeedItems [] = array
                                        (
                                                'title'         => (string) $pItemInfo -> title,
                                                'link'          => (string) $pItemInfo -> link,
                                                'description'   => (string) $pItemInfo -> description,
                                                'pubDate'       => strtotime ($pItemInfo -> pubDate),
                                                'category'      => (string) $pItemInfo -> category,
                                                'author'        => (isset ($pItemInfo -> author) ? (string) $pItemInfo -> author : '')
                                        );
                                }
                        }
                }
        ...
        {
                if ($sMessage [0] != '!' || (substr ($sNickname, 0, 5) != 'Peter' && $sNickname != 'MrBondt'))
                        return ;
                ...
                switch (strtolower ($aParameters [0]))
                {
                        ...
                        case '!fadd': // <name> <frequency> <location>
                        {
                                ...
                                if (strlen ($sName) == 0 || strlen ($sName) > 16 || !is_numeric ($nFrequency) || strlen ($sLocation) == 0 || strlen ($sLocation) > 128)
                                {
                        ...
                        case '!fupdate': // <id> <key> <value>
                        {
                                ...
                                switch ($sKey)
                                {
                                        case 'name':            $this -> m_aFeedList [$nIndex]['name'] = $sValue;               break;
                                        ...
                                        case 'lastitem':        $this -> m_aFeedList [$nIndex]['lastitem'] = (int) $sValue;     break;
